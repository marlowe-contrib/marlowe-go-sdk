/*
Marlowe Runtime REST API

REST API for Marlowe Runtime

API version: 0.0.5.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package marloweruntime

import (
	"encoding/json"
)

// checks if the GreaterOrEqualObject type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GreaterOrEqualObject{}

// GreaterOrEqualObject struct for GreaterOrEqualObject
type GreaterOrEqualObject struct {
	GeThan ValueObject `json:"ge_than"`
	Value ValueObject `json:"value"`
}

// NewGreaterOrEqualObject instantiates a new GreaterOrEqualObject object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGreaterOrEqualObject(geThan ValueObject, value ValueObject) *GreaterOrEqualObject {
	this := GreaterOrEqualObject{}
	this.GeThan = geThan
	this.Value = value
	return &this
}

// NewGreaterOrEqualObjectWithDefaults instantiates a new GreaterOrEqualObject object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGreaterOrEqualObjectWithDefaults() *GreaterOrEqualObject {
	this := GreaterOrEqualObject{}
	return &this
}

// GetGeThan returns the GeThan field value
func (o *GreaterOrEqualObject) GetGeThan() ValueObject {
	if o == nil {
		var ret ValueObject
		return ret
	}

	return o.GeThan
}

// GetGeThanOk returns a tuple with the GeThan field value
// and a boolean to check if the value has been set.
func (o *GreaterOrEqualObject) GetGeThanOk() (*ValueObject, bool) {
	if o == nil {
		return nil, false
	}
	return &o.GeThan, true
}

// SetGeThan sets field value
func (o *GreaterOrEqualObject) SetGeThan(v ValueObject) {
	o.GeThan = v
}

// GetValue returns the Value field value
func (o *GreaterOrEqualObject) GetValue() ValueObject {
	if o == nil {
		var ret ValueObject
		return ret
	}

	return o.Value
}

// GetValueOk returns a tuple with the Value field value
// and a boolean to check if the value has been set.
func (o *GreaterOrEqualObject) GetValueOk() (*ValueObject, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Value, true
}

// SetValue sets field value
func (o *GreaterOrEqualObject) SetValue(v ValueObject) {
	o.Value = v
}

func (o GreaterOrEqualObject) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GreaterOrEqualObject) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["ge_than"] = o.GeThan
	toSerialize["value"] = o.Value
	return toSerialize, nil
}

type NullableGreaterOrEqualObject struct {
	value *GreaterOrEqualObject
	isSet bool
}

func (v NullableGreaterOrEqualObject) Get() *GreaterOrEqualObject {
	return v.value
}

func (v *NullableGreaterOrEqualObject) Set(val *GreaterOrEqualObject) {
	v.value = val
	v.isSet = true
}

func (v NullableGreaterOrEqualObject) IsSet() bool {
	return v.isSet
}

func (v *NullableGreaterOrEqualObject) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGreaterOrEqualObject(val *GreaterOrEqualObject) *NullableGreaterOrEqualObject {
	return &NullableGreaterOrEqualObject{value: val, isSet: true}
}

func (v NullableGreaterOrEqualObject) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGreaterOrEqualObject) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


